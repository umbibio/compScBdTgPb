...
d0.meta <- pru.meta[d0.ind, ] %>% transmute(Sample = index, dpi = dpi, phase = cell_cycle)
...
d3.meta <- pru.meta[d3.ind, ] %>% transmute(Sample = index, dpi = dpi, phase = cell_cycle)
...
d5.meta <- pru.meta[d5.ind, ] %>% transmute(Sample = index, dpi = dpi, phase = cell_cycle)
...
d7.meta <- pru.meta[d7.ind, ] %>% transmute(Sample = index, dpi = dpi, phase = cell_cycle)
...
saveRDS(S.O.list, '../Input/compScBdTgPb/RData/S.O.tg.pru.boothroyd.list.rds')
...
Idents(S.Os.integrated) <- "seurat_clusters"
DimPlot(S.Os.integrated, reduction = 'umap')
Idents(S.Os.integrated) <- "phase"

S.O.scale <- scale.S.O(S.Os.integrated)
DefaultAssay(S.O.scale) <- 'scale'
i <- 12
FeaturePlot(S.O.scale, features = my.genes[i])
