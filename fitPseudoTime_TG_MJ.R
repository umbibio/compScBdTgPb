...
S.O.tg <- readRDS('../Input/compScBdTgPb/RData/S.O.toxo_MJ_lables.RData')
S.O.tg@meta.data$phase <- S.O.tg@meta.data$predicted.id

## Find phase markers
Idents(S.O.tg) <- 'phase'
markers <- FindAllMarkers(object = S.O.tg, only.pos = TRUE, min.pct = 0.25) 
...
pc.sds.tg$phase <- S.O.tg@meta.data$phase[match(pc.sds.tg$Sample, rownames(S.O.tg@meta.data))]
pc.sds.tg$phase <- factor(pc.sds.tg$phase, levels = c('G1.a', 'G1.b', 'S', 'M', 'C'))

saveRDS(pc.sds.tg, '../Input/compScBdTgPb/RData/pc.sds.tg_new.RData')
...
ggsave(filename="../Output/compScBdTgPb/figs/initial_pseudo_time_TG_new.pdf",
       plot=p,
       width = 5, height = 5,
...
pc <- S.O.tg[['pca']]@cell.embeddings
pc <- data.frame(pc) %>% dplyr::mutate(Sample = rownames(pc))
pc$cluster <- S.O.tg$cluster
