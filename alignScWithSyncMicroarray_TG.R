...
cell.cycle.genes.df <- readRDS('../Input/compScBdTgPb/RData/tg_cell_cycle_genes_df.RData')
tc_expr <- readRDS('../Input/compScBdTgPb/RData/tg_tc_sync_expr.RData')
S.O.tg.filt <- readRDS('../Input/compScBdTgPb/RData/S.O.tg.RData')
...
saveRDS(comm.genes, '../Input/compScBdTgPb/RData/tg_sync_sc_comm.genes.RData')
...
saveRDS(mu.sync.com.grid, '../Input/compScBdTgPb/RData/tg_mu.sync.com.grid.RData')
saveRDS(mu.sc.com.grid, '../Input/compScBdTgPb/RData/tg_mu.sc.com.grid.RData')
...
plot(p)  
ggsave(filename="../Output/compScBdTgPb/figs/tg_lag_time_dist.pdf",
       plot=p,
       width = 5, height = 4,
...
lag.time <- hh$breaks[which.max(hh$counts)] + 2 ## Seems like the lag time matches. No need to adjust.
...
pc <- S.O.tg.filt[['pca']]@cell.embeddings
pc <- data.frame(pc) %>% dplyr::mutate(Sample = rownames(pc))
pc$cluster <- S.O.tg.filt$phase
...
saveRDS(pc.sds.adj, '../Input/compScBdTgPb/RData/pc.sds.adj.RData')
...
plot(p)
ggsave(filename="../Output/compScBdTgPb/figs/aligned_pseudo_time_TG.pdf",
       plot=p,
       width = 5, height = 5,
       units = "in", # other options are "in", "cm", "mm"
       dpi = 300
...
saveRDS(cell.cycle.genes.df.adj, '../Input/compScBdTgPb/RData/bd_cell_cycle_genes_df_adj.RData')

sc.tc.df.adj <- cell.cycle.genes.df.adj %>% 
  transmute(y = log2.expr, tme = adj.time, ind = rep.y, variable = GeneID)

saveRDS(sc.tc.df.adj, '../Input/compScBdTgPb/RData/bd_sc_tc_df_adj.RData')
