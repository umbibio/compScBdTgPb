...
enhance = function(
  ...
  pca_raw = perform_pca(D = as.matrix(data_raw), med = med_raw, return_pcs = TRUE, ratio_pcs = ratio_pcs)
  ...
  data_raw_pca_agg1 = t(t(transform_ft(normalize_median(as.matrix(data_raw), med = med_raw)) - pca_agg1$center) %*% 
                          pca_agg1$rotation)[pca_raw$pcs, ]
  ...
  data_agg2 = aggregate_nearest_neighbors(D = as.matrix(data_raw), nn = nn_2)
  sizes_agg2 = estimate_sizes(D = as.matrix(data_raw), nn = nn_2)
